...
export function Footer() {
  return (
    <footer className="mt-24 border-t border-border bg-secondary/40">
      <div className="container mx-auto grid gap-10 px-4 py-14 lg:grid-cols-4 lg:px-8 text-[15px]">
        <div>
          <div className="flex items-center gap-3">
            <img src={logo} alt="Dr Shahid's Child & Neuro Clinic" width={72} height={72} className="h-16 w-16 rounded-full object-contain bg-white shadow-soft" />
            <div>
              <div className="font-display font-bold text-base">{SITE.short}</div>
              <div className="text-xs text-muted-foreground">{SITE.qualification}</div>
            </div>
          </div>
          <p className="mt-4 text-[15px] text-muted-foreground leading-relaxed">{SITE.tagline}</p>
        ...
        <div>
          <h4 className="font-display font-semibold text-base mb-4">Visit Clinic</h4>
          <p className="text-[15px] text-muted-foreground mb-3">{SITE.clinic}, Rawalpindi</p>
          <a href={waLink()} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full gradient-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-soft hover:shadow-elegant transition-smooth">
